...
// R4 v2 patch: the `js_protocol::runtime` module import was dropped here — its
// only use was `runtime::EnableParams::default()` in `init_commands`, which the
// patch removes. The runtime *event* types are still pulled in via the glob
// import on the line above (`js_protocol::runtime::*`).
...
impl FrameManager {
    ...
    pub fn init_commands(timeout: Duration) -> CommandChain {
        // ============================================================
        // R4 v2 patch (web_search_mcp vendored fork of chromiumoxide 0.7.0)
        // ------------------------------------------------------------
        // REMOVED the eager `Runtime.enable` (`runtime::EnableParams`) that
        // upstream queued as the 4th command of this init chain.
        //
        // WHY: `Runtime.enable` is auto-fired at target init, BEFORE any user
        // code runs. Cloudflare / DataDome / sannysoft and the rebrowser
        // bot-detector key on this exact load-time CDP signal (the
        // `Runtime.enable -> Runtime.executionContextCreated /
        // consoleAPICalled` sequence) to fingerprint an automated/CDP-driven
        // browser. It is NOT suppressible through chromiumoxide's public API
        // (R-1 investigation, ADR 0003 §7 Option R4-a / Design 0004 §0,§7),
        // so removing it here in the vendored crate is the only fix.
        //
        // WHAT STILL WORKS: `Runtime.evaluate` is a *command*, not an event
        // subscription — it runs fine WITHOUT `Runtime.enable`, defaulting to
        // the page's main execution context when no `contextId` is supplied.
        // `Page::evaluate` already passes `contextId: None` when
        // `execution_context()` is unknown (page.rs ~1037-1038), and with the
        // runtime event stream off `execution_context()` simply stays `None`,
        // so `evaluate()` / `content()` transparently target the default
        // context. The trade-off: the typed `executionContextCreated` /
        // `executionContextDestroyed` event tracking and the eager
        // utility-isolated-world context capture no longer populate
        // (`secondary_execution_context()` returns `None`). The crawler's SPA
        // render + stealth paths do not depend on that typed tracking — they
        // use `content()`, `goto()`, `wait_for_navigation()` (Page lifecycle,
        // KEPT below) and `get_cookies()`. See vendor/chromiumoxide/PATCH_NOTES.md.
        //
        // KEPT: `Page.enable`, `Page.getFrameTree`,
        // `Page.setLifecycleEventsEnabled(true)` — these drive the frame tree
        // and the lifecycle events that `wait_for_navigation()` relies on, and
        // are not the load-time leak detectors fingerprint.
        // ============================================================
        ...
        // R4 v2: `runtime::EnableParams` intentionally NOT constructed/queued.
        CommandChain::new(
            vec![
                ...
                // R4 v2 patch: the `Runtime.enable` entry that upstream queued
                // here is removed — it is the load-time CF/DataDome CDP leak.
            ],
            timeout,
        )
    }
